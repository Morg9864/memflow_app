...
class CsvImportService {
  ...
  CsvImportPreview parse(List<int> bytes) {
    ...
    if (rows.isEmpty) {
      return const CsvImportPreview(
        cards: [],
        issues: [CsvImportIssue(rowNumber: 0, message: 'Le fichier CSV est vide.')],
        delimiter: ';',
        withHeader: true,
      );
    }
    ...
    for (var index = startIndex; index < rows.length; index++) {
      ...
      if ([collection, deck, question, correctAnswer, ...wrongAnswers]
          .any((value) => value.isEmpty)) {
        issues.add(CsvImportIssue(
          rowNumber: rowNumber,
          message:
      ...
      final difficulty = _parseDifficulty(_readValue(row, 'difficulty', 10, headerMap));
      if (difficulty == null) {
        issues.add(CsvImportIssue(
          rowNumber: rowNumber,
      ...
      final clozeText = _emptyToNull(_readValue(row, 'cloze_text', 13, headerMap));
      cards.add(
        CsvImportCardDraft(
          ...
          explanation: _emptyToNull(_readValue(row, 'explanation', 8, headerMap)),
          level: level,
  ...
  String _detectDelimiter(String raw) {
    ...
    return delimiters.reduce((best, candidate) {
      final bestCount = best == '\t' ? '\t'.allMatches(sample).length : best.allMatches(sample).length;
      final candidateCount =
          candidate == '\t' ? '\t'.allMatches(sample).length : candidate.allMatches(sample).length;
  ...
  bool _looksLikeHeader(List<String> row) {
    ...
    return normalized.contains('collection') && normalized.contains('correct_answer');
  }
  ...
  String _readValue(List<String> row, String key, int fallbackIndex, Map<String, int> headerMap) {
